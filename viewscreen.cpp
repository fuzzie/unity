...
void ViewscreenScreen::start() {
	...
	obj->sprite = new SpritePlayer(new Sprite(_vm->data.openFile(sprFilename)), obj, _vm);
	...
	obj->flags = OBJFLAG_ACTIVE;
	obj->objwalktype = OBJWALKTYPE_NORMAL;
	obj->sprite = new SpritePlayer(new Sprite(_vm->data.openFile("viewscan.spr")), obj, _vm);
	obj->sprite->startAnim(0);
	_vm->data._currentScreen.objects.push_back(obj);
